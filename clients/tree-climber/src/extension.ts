...
export function activate(context: vscode.ExtensionContext) {
  const disposable = vscode.commands.registerCommand('tree-climber.splitStr', async () => {
    ...
    const result = await makeOperationRequest({ line: cursor.line, char: cursor.character, content: info.text }) as {start: { row: number, column: number}, end: {row: number, column: number}, content: string }
    applyPatch({content: result.content, end: {character: result.end.column, line: result.end.row,}, start: { character: result.start.column, line: result.start.row }});
  });
...
function makeOperationRequest(params: {line: number, char: number, content: string }) {
  const API_URL = 'http://localhost:4434';

  const content = `const foo = "bar"`;
  const point = { row: params.line, col: params.char };
  const payload = {command: 'splitExpr', point: { row: point.row, column: point.col }, content: params.content};

  return fetch(`${API_URL}/operation`, { method: 'POST', body: JSON.stringify(payload) }).then(res => {
    if(!res.ok) {
      throw res;
    }
...
export async function applyPatch(patch: Patch) {
  const editor = vscode.window.activeTextEditor;
  if (!editor) { vscode.window.showWarningMessage('No active text editor.'); return; }
  ...
  if (!ok) {vscode.window.showErrorMessage('Could not apply patch.');}
  else {
